...
public class ChatService {
    ...
    public List<Chat> getAllChatUser(Long userID){
       return chatRepository.getAllChat().stream().filter(chat -> chat.getRecipientId() == userID || chat.getSenderId() == userID).collect(Collectors.toList());
    }

    public Chat getChatById(Long chatId){
       return chatRepository.getAllChat().stream().filter(chat -> chat.getId() == chatId).findAny().orElse(null);
    }
    public void addChat(Chat chat){
        chatRepository.addChat(chat);
    }
    public void addMessageInChat(Long chatId, Message message){
        chatRepository.getAllChat().stream().filter(chat -> chat.getId() == chatId).findAny().get().addMessageInChat(message);
    }
